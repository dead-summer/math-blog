...
#show: main-zh.with(
  ...
  author: "summer",
  desc: [证明线弹性与 Kirchhoff--Love 板弯曲的混合最小二乘泛函与相应图范数平方双边等价，等价常数可关于 Lamé 常数一致选取。本文还将参数取准均匀点集的单隐层线性化 ReLU 幂次网络用于两类方程的求解，给出误差分析与完整算法的数值收敛结果],
  date: "2026-04-20",
  tags: (
...
线性化网络方法预先取定并冻结单隐层参数，随后只求输出层系数。冻结参数生成的特征族在逼近论中称为字典（dictionary）。给定字典后，偏微分方程离散仍是有限维凸二次问题。本文沿用 @SiegelHongJinHaoXu2023 的字典参数化，将单隐层参数取为方向--偏置参数域上的确定性准均匀点集，而非随机抽样。本文将这一结构用于线弹性应力--位移系统与 Kirchhoff--Love 板弯曲弯矩--挠度系统，连续问题采用经典的混合最小二乘泛函。
...
$
  rho_k (t):=max(t, 0)^k,
$
其中整数 $k >= max(m, 2) + 1$，$m$ 为相应图范数所含的最高导数阶（线弹性 $m=1$，板弯曲 $m=2$）。该下界保证 $rho_k in W^(k,oo)(RR) subset W^(m+1,oo)(RR)$，后者是 @thm:train-generalization 中 Rademacher 估计所需的正则性。单隐层特征即 $rho_k$ 与仿射函数的复合
$
  bold(x) |-> rho_k (bold(omega) dot bold(x) + b),
  quad bold(omega) in RR^d without {bold(0)},
  quad b in RR,
$
其中 $bold(omega)$ 称为方向，$b$ 称为偏置，被冻结的单隐层参数即 $(bold(omega), b)$。该函数沿 $bold(omega)$ 变化，在与 $bold(omega)$ 正交的方向上取常值，这类函数称为脊函数（ridge function）。方向--偏置的参数域取柱面 $bb(S)^(d-1) times [-c_b, c_b]$，其中 $bb(S)^(d-1) subset RR^d$ 为单位球面，偏置半径 $c_b$ 为取定常数，满足
$
...
$
由 $Omega$ 有界，这样的 $c_b$ 存在。记 $overline(Omega)$ 在方向 $bold(omega)$ 上的支撑区间端点为
$
...
$
按超平面 ${bold(omega) dot bold(x) + b = 0}$ 与 $overline(Omega)$ 的位置关系，特征在 $Omega$ 上的限制分三种情形：
$
  rho_k (bold(omega) dot bold(x) + b)|_Omega =
  cases(
    0 in P_k (Omega)\, & quad b <= b_-(bold(omega)),
    max(bold(omega) dot bold(x) + b, 0)^k\, & quad (bold(omega), b) in cal(P)_Omega,
    (bold(omega) dot bold(x) + b)^k in P_k (Omega)\, & quad b >= b_+(bold(omega)).
  )
$
<eq:feature-trichotomy>
中间一段即非退化参数：$(bold(omega), b) in cal(P)_Omega$ 当且仅当仿射函数 $bold(omega) dot bold(x) + b$ 在 $overline(Omega)$ 上变号，等价地，超平面与 $overline(Omega)$ 相交。两端则退化：由 $b_+$ 的定义，$b >= b_+(bold(omega))$ 时对一切 $bold(x) in overline(Omega)$ 有 $bold(omega) dot bold(x) + b >= bold(omega) dot bold(x) - min_(bold(y) in overline(Omega)) bold(omega) dot bold(y) >= 0$，截断不起作用，特征化为 $k$ 次多项式；同理 $b <= b_-(bold(omega))$ 时 $bold(omega) dot bold(x) + b <= 0$，特征恒为零。因此只有非退化参数生成脊特征，退化参数的特征都落在 $P_k (Omega)$ 中。
...
在球面坐标下，非退化参数集之外的特征只贡献多项式。

#lemma(title: [退化区分解])[
  记 $cal(G) := bb(S)^d without frak(P)(cal(P)_Omega)$。则对任意 $bold(theta) in cal(G)$，$rho_k (bold(theta) dot tilde(bold(x)))|_Omega in P_k (Omega)$。从而对任意 $psi in L^2(bb(S)^d)$，
  $
    integral_(cal(G)) rho_k (bold(theta) dot tilde(bold(x))) psi(bold(theta)) dif bold(theta)
    in P_k (Omega).
  $
]<lem:degenerate-split>

#proof[
  设 $bold(theta) = frak(P)(bold(omega), b) in cal(G)$。由 $frak(P)$ 是双射且 $rho_k$ 正 $k$ 次齐次，只需就 $(bold(omega), b) in.not cal(P)_Omega$ 讨论。此时 $b >= b_+(bold(omega))$ 或 $b <= b_-(bold(omega))$，由 @eq:feature-trichotomy 的首末两式，该特征在 $Omega$ 上分别等于 $k$ 次多项式与零，两种情形都落在 $P_k (Omega)$ 中。

  于是 $bold(theta) |-> rho_k (bold(theta) dot tilde(bold(x)))|_Omega$ 在 $cal(G)$ 上是取值于 $P_k (Omega)$ 的映射，且被 $(sup_(bold(x) in overline(Omega)) norm(bold(x))_(ell^2) + c_b)^k$ 一致控制。$P_k (Omega)$ 是 $L^2(Omega)$ 的有限维子空间，因而闭。由 Bochner 积分保持闭子空间，上述积分仍属于 $P_k (Omega)$。证毕。
]

由 @lem:degenerate-split，退化区上的特征只贡献 $P_k (Omega)$ 中的多项式，这一有限维空间可由固定的有限个特征补足，故参数点集在退化区中的分布不影响逼近，其分布质量只需在 $frak(P)(cal(P)_Omega)$ 上度量。
...
#definition(title: [准均匀参数点集])[
  ...
  $
  其中 $op("dist")$ 为 $bb(S)^d$ 上的测地距离。称 $Theta$ 关于 $N$ 准均匀，若其在 $frak(P)(cal(P)_Omega)$ 上良分布，
  $
...
以下两条定理给出字典在球面上的逼近性质，第一条刻画字典可利用的光滑性上限。
...
#theorem(title: [球面积分表示与饱和指数（@LiuMaoXu2025 定理 2.3--2.4）])[
  ...
  $
    ...
    tilde.eq inf {norm(psi)_(L^2(bb(S)^d)) :
      ...
      quad bold(x) in Omega}.
  $
  该刻画充分必要：在某个与 $N$ 无关的系数预算下可被准均匀字典按 $L^2$ 逼近的函数，恰为 $H^(s_"cap" (d))(Omega)$ 的元素。
...
第二条给出逼近率，其假设即对参数点集的全部要求。

#theorem(title: [准均匀字典的 Sobolev 逼近率（@LiuMaoXu2025 定理 2.2）])[
  设 $0 <= m <= k$，有限参数集 $Theta = Theta^"poly" union.sq Theta^"act" subset bb(S)^(d-1) times [-c_b, c_b]$ 含 $N$ 对参数，其生成的特征连同常数特征 $xi_0 := 1$ 记为 ${xi_j}_(j=0)^N$。设
  - (H1) $Theta^"poly"$ 固定且与 $N$ 无关，其生成的特征与 $xi_0$ 一并张成 $P_k (Omega)$；
  - (H2) $Theta^"act" subset cal(P)_Omega$ 按 @def:quasi-uniform 关于 $N$ 准均匀，覆盖半径记为 $h$。

  则对任意 $s in [m, s_"cap" (d)]$ 与 $v in H^s (Omega)$，存在系数 $(c_j)_(j=0)^N$，使 $v_N := sum_(j=0)^N c_j xi_j$ 满足
  $
    norm(v - v_N)_(H^m (Omega)) & lt.tilde h^(s - m) norm(v)_(H^s (Omega)), \
     (sum_(j=0)^N c_j^2)^(1\/2) & lt.tilde h^(-(s_"cap" (d) - s))
                                  norm(v)_(H^s (Omega)) N^(-1\/2).
  $
  两处隐含常数依赖于 $Omega$、$d$、$k$、$m$、$c_b$、@def:quasi-uniform 中的常数与 $Theta^"poly"$ 的取法，不依赖于 $N$ 与 $v$。
]<thm:dictionary-rate>

@LiuMaoXu2025 定理 2.2 中的退化集为与方向无关的极冠 ${bold(theta) in bb(S)^d : abs(theta_(d+1)) >= "diam"(Omega)\/sqrt(1 + "diam"(Omega)^2)}$，$P_k (Omega)$ 由 $dim P_k (Omega)$ 个退化区中的脊特征张成。其证明只通过两条途径依赖这两个取法：退化集上的特征属于 $P_k (Omega)$，以及固定张成组只经由与 $N$ 无关的过渡矩阵进入系数界。前者对逐方向精确非退化区间之外的 $cal(G)$ 依然成立，即 @lem:degenerate-split；后者的过渡矩阵由 $Theta^"poly"$ 确定而与 $N$ 无关，故系数界中至多相差与 $N$ 无关的常数，定理按 (H1)、(H2) 的形式成立。

=== 参数点集构造

以 $chi$ 标记物理变量：线弹性中 $chi in {bold(sigma), bold(u)}$，板弯曲中 $chi in {bold(M), w}$。每个物理变量各自生成单隐层参数，即取含 $N$ 对 $(bold(omega), b)$ 的确定性点集 $Theta_(chi,N) subset bb(S)^(d-1) times [-c_b, c_b]$，并按 @thm:dictionary-rate 的分块作不交分解
$
  Theta_(chi,N) = Theta^"poly" union.sq Theta_(chi,N)^"act", quad Theta^"poly" inter Theta_(chi,N)^"act" = emptyset,
$
其中 $Theta^"poly"$ 是退化区中固定的 $dim P_k (Omega) - 1$ 对参数，与 $chi$、$N$ 均无关，下称多项式方向；$Theta_(chi,N)^"act" subset cal(P)_Omega$ 是其余 $N - dim P_k (Omega) + 1$ 对。因此 $abs(Theta_(chi,N)^"act") tilde.eq N$，下文以 $N$ 的幂次表述的量级换用 $abs(Theta_(chi,N)^"act")$ 表述时只差与 $N$ 无关的常数。其中隐含常数依赖于 $k$：多项式补充的大小 $dim P_k (Omega) = binom(d+k, k)$ 按 $k^d$ 增长而与 $N$ 无关。记 $Theta_(chi,N)$ 的元素为 $(bold(omega)_(chi,j), b_(chi,j))$，$1 <= j <= N$，变量 $chi$ 的特征与常数特征分别为
$
  xi_(chi,j) (bold(x)) := rho_k (bold(omega)_(chi,j) dot bold(x) + b_(chi,j)),
  quad xi_(chi,0) := 1.
$

假设 (H1) 由 $Theta^"poly"$ 的取法给出。常数特征 $xi_(chi,0)$ 本身不是脊特征，故多项式方向只需 $dim P_k (Omega) - 1$ 对：退化区 $b >= b_+(bold(omega))$ 中的脊特征张成 $P_k (Omega)$，从中取出与 $xi_(chi,0)$ 一并构成 $P_k (Omega)$ 一组基的 $dim P_k (Omega) - 1$ 个即可。

这组脊特征的选取影响的是数值条件而非张成空间。实现中对每个方向按其精确的退化阈值 $b_+(bold(omega))$ 取偏置，再在 $L^2(Omega)$ 度量下作列主元 QR：偏置贴近 $b_+(bold(omega))$ 时超平面与 $overline(Omega)$ 相切，$(bold(omega) dot bold(x) + b)^k$ 在 $Omega$ 上的变化最大，所得基的 $L^2$ 条件数比按方向无关的阈值取偏置低 $2$--$3$ 个数量级（$k=7$ 时 $d=2$ 降约 $500$ 倍、$d=3$ 降约 $6600$ 倍）。

假设 (H2) 是对 $Theta_(chi,N)^"act"$ 的要求。本文取每个 $Theta_(chi,N)^"act"$ 关于 $N$ 准均匀，并记 $h_(chi,N) := h(Theta_(chi,N)^"act")$，构造分层进行：将偏置换成其在非退化区间 $(b_-(bold(omega)), b_+(bold(omega)))$ 中的相对位置
$
  t(bold(omega), b)
  := (b - b_-(bold(omega)))/(b_+(bold(omega)) - b_-(bold(omega))) in (0, 1),
$
将 $(0,1)$ 等分为 $n_2 tilde.eq N^(1\/d)$ 段并取各段中点 $t_k := (k - 1\/2)\/n_2$，$1 <= k <= n_2$。第 $k$ 层配置各自的准均匀方向集 $Lambda_k subset bb(S)^(d-1)$，各层点数至多相差 $1$，均为 $n_1 tilde.eq N^((d-1)\/d)$。方向 $bold(omega) in Lambda_k$ 所配的偏置由 $t(bold(omega), b) = t_k$ 反解，即 $b(bold(omega), t_k) := b_-(bold(omega)) + t_k (b_+(bold(omega)) - b_-(bold(omega)))$，第 $k$ 层的参数为 ${(bold(omega), b(bold(omega), t_k)) : bold(omega) in Lambda_k}$。方向集按维数取：
...
#figure(
  code-image(class: "center", theme => diagram(
    ...
    edge(<prod>, <cyl>, $(bold(omega), t) |-> (bold(omega), b)$, "->", stroke: 0.6pt + theme.main-color),
  ...
  caption: [参数域的三套坐标：点集在乘积坐标中构造，准均匀性则按 @def:quasi-uniform 在 $bb(S)^d$ 上度量。两段映射均双向 Lipschitz，常数只依赖于 $Omega$ 与 $c_b$，故覆盖半径与最小间距在两端各差至多一个常数因子],
)<fig:least-squares-param-charts>

#proposition(title: [分层点集的准均匀性])[
  按上述构造取 $Theta_(chi,N)^"act" = union.big_(k=1)^(n_2) {(bold(omega), b(bold(omega), t_k)) : bold(omega) in Lambda_k}$，则其满足 @def:quasi-uniform，即 $h_(chi,N) tilde.eq N^(-1\/d)$，且覆盖半径与最小间距同阶。
]<prop:layered-quasi-uniform>

#proof[
  先将两条要求移到乘积坐标 $bb(S)^(d-1) times [0, 1]$ 上。由 $b = b_-(bold(omega)) + t (b_+(bold(omega)) - b_-(bold(omega)))$，映射 $(bold(omega), t) |-> (bold(omega), b)$ 关于 $t$ 仿射，斜率为方向宽度 $b_+(bold(omega)) - b_-(bold(omega))$。该宽度关于 $bold(omega)$ 有正的上下界（$Omega$ 含一个半径 $r > 0$ 的球时其不小于 $2 r$，又不超过 $"diam"(Omega)$；$Omega = (0,1)^d$ 时其等于 $norm(bold(omega))_(ell^1) in [1, sqrt(d)]$），而 $b_+$ 与 $-b_-$ 分别是 $overline(Omega)$ 关于 $-bold(omega)$ 与 $bold(omega)$ 的支撑函数，以 $max_(bold(x) in overline(Omega)) norm(bold(x))_(ell^2)$ 为 Lipschitz 常数，故该映射是 $bb(S)^(d-1) times [0,1]$ 到 $overline(cal(P))_Omega$ 的双向 Lipschitz 同胚。与 $frak(P)$ 复合后，$bb(S)^(d-1) times [0,1]$ 到 $frak(P)(overline(cal(P))_Omega)$ 的映射仍双向 Lipschitz，常数只依赖于 $Omega$ 与 $c_b$（@fig:least-squares-param-charts）。双向 Lipschitz 映射将覆盖半径与最小间距同时改变至多一个常数因子，故两条要求只需在乘积度量下验证。

  在乘积度量下，层距为 $n_2^(-1) tilde.eq N^(-1\/d)$，同层内的间距为 $Lambda_k$ 在 $bb(S)^(d-1)$ 上的球面间距，由其准均匀性亦为 $n_1^(-1\/(d-1)) tilde.eq N^(-1\/d)$ 量级。覆盖半径由这两个量级的较大者控制，故 $h_(chi,N) lt.tilde N^(-1\/d)$。最小间距同样是该量级：同层内为 $Lambda_k$ 的球面间距，跨层则不小于层距。两者同阶即得 @def:quasi-uniform 的两条要求。证毕。
]

至此 @thm:dictionary-rate 的两条假设对每个 $Theta_(chi,N)$ 成立，该定理适用于字典 ${xi_(chi,j)}_(j=0)^N$，覆盖半径取 $h_(chi,N) tilde.eq N^(-1\/d)$。
...
将给定模型的全部独立输出系数依固定顺序排成向量 $bold(c) in RR^(m_N)$，其中 $m_N$ 是离散系数总数，即上述原始字典空间的维数之和：线弹性为 $m_N=(n_s+d)(N+1)$，板弯曲为 $m_N=4(N+1)$。记 $bold(c)$ 的应力（板弯曲为弯矩）块系数为 $(c_(j,alpha))_(0<=j<=N, 1<=alpha<=n_s)$、位移块系数为 $(c_(j,i))_(0<=j<=N, 1<=i<=d)$，并将 $bold(c)$ 在原始空间中确定的场记为
$
...
#proof[
  ...
  $
  其中 $C_xi <= (sup_(bold(x) in overline(Omega)) norm(bold(x))_(ell^2) + c_b)^k$ 是与 $N$ 无关的常数。综合三式，并用应力特征数 $N + 1 <= m_N$，得对平均迹规则的二阶矩界
  $
...
#corollary(title: [盒状区域上的精确平均迹投影])[
  ...
  $
  <eq:box-mean>
  其中 $rho_1(t) := max(t, 0)$。退化参数的两种情形分别取 $k$ 次多项式在盒上的精确均值与零。以 @eq:box-mean 的精确均值构造 $tilde(Pi)_("tr",Q_"tr")$ 中被减去的常数球张量，则
  $
...
#proof[
  先设 $(bold(omega), b) in cal(P)_Omega$。对 $omega_i < 0$ 的坐标作反射 $x_i |-> 1 - x_i$，盒不变而仿射函数化为 $sum_i abs(omega_i) x_i + tilde(b)$。零分量对应的坐标不出现在被积函数中，积分消去后只剩 $n$ 个活动方向。对这 $n$ 个方向逐一取原函数：每积一次使 $rho_k$ 的幂次增加 $1$，并除以相应的 $abs(omega_i)$ 与新幂次，$n$ 次之后得到幂次 $k+n$、分母 $product abs(omega_i) dot (k+n)!\/k!$，而按盒的顶点取值作容斥即得 @eq:box-mean 的交错和。

  再考虑退化参数。由 @lem:degenerate-split 的两种情形：$b >= b_+(bold(omega))$ 时特征等于 $k$ 次多项式 $(bold(omega) dot bold(x) + b)^k$；$b <= b_-(bold(omega))$ 时特征在 $Omega$ 上恒为零，均值为零。第一种情形当然也可代入 @eq:box-mean，但该式中的交错和在被积函数不变号时会出现严重相消。实现中改用每轴 $ceil((k+1)\/2)$ 点的张量 Gauss--Legendre 规则，该规则对 $k$ 次多项式精确，故均值仍无求积误差。
...
#proof[
  ...
  $
  由于正交投影非扩张，右端每个残差不超过
  $norm(xi_(v,j))_(H^m)$。$rho_k$ 的参数域有界，故这些特征的 $H^m$ 范数关于 $j$ 与 $N$ 一致有界。再用 $n_v(N+1)<=m_N$，并以
...
字典逼近误差由 @thm:dictionary-rate 给出，其中的饱和指数为
$
  s_"cap" (d) = (d+2k+1)/2.
$
...
将 @thm:dictionary-rate 施于变量 $chi$ 的每个标量分量 $v_chi in H^(s_chi)(Omega)$，并按 @prop:layered-quasi-uniform 代入覆盖半径 $h_(chi,N) tilde.eq N^(-1\/d)$，即得原始字典近似 $hat(v)_N := sum_(j=0)^N c_j xi_(chi,j)$ 满足
$
  norm(v_chi - hat(v)_N)_(H^m (Omega)) & lt.tilde N^(-(s_chi - m)\/d) norm(v_chi)_(H^(s_chi)(Omega)), \
            (sum_(j=0)^N c_j^2)^(1\/2) & lt.tilde N^((s_"cap" (d) - s_chi)\/d - 1\/2)
                                         norm(v_chi)_(H^(s_chi)(Omega)),
$
其中 $(c_j)_(j=0)^N$ 是单个标量分量在变量 $chi$ 的字典上的系数。对各物理变量取最慢的收敛阶，整体字典逼近误差因而为 $N^(-beta)$ 阶，其中 $beta=beta_"LE"$ 或 $beta_"KL"$。在解足够光滑、逼近阶在饱和处取到的情形下，将 $s_chi = s_"cap" (d)$ 代入即得本文反复使用的显式速率
$
...
- 其一，投影前后的两个函数类并不互相接近。由 $Theta_(chi,N)^"act" subset cal(P)_Omega$，仿射函数 $bold(omega)_(chi,j) dot bold(x) + b_(chi,j)$ 在 $overline(Omega)$ 上变号，其最大值因而为正。该函数梯度非零，最大值只在 $partial Omega$ 上取到。于是 $xi_(chi,j)$ 在 $partial Omega$ 的某点严格为正，迹不恒为零。$Omega$ 为 Lipschitz 区域，$H_0^m (Omega) subset H_0^1 (Omega)$ 中的元素迹为零，故 $xi_(chi,j) in.not H_0^m (Omega)$，而 $H_0^m (Omega)$ 闭，从而
  $
    norm(xi_(chi,j) - Pi_D^(m) xi_(chi,j))_(H^m (Omega))
    = op("dist")_(H^m (Omega)) (xi_(chi,j), H_0^m (Omega))
    > 0.
  $
  该距离由 $(bold(omega)_(chi,j), b_(chi,j))$ 与 $Omega$ 决定，投影因而不是恒等算子的扰动。已投影块随之不再是脊函数族的像，@SiegelHongJinHaoXu2023 定理 6 所依赖的参数覆盖数论证在其上无从施行。
...
分块由两个投影的作用范围给出：$tilde(Pi)_("tr",Q_"tr")$ 只作用于应力（弯矩）块，$tilde(Pi)_(D,K,Q_"R")^(m)$ 只作用于位移（挠度）块且逐分量作用，故后者只生成 $N+1$ 个标量函数，与分量指标无关。据此将投影基场的全部标量分量分成两族，线弹性为
$
  cal(B)_("LE",N,K) & := {(tilde(Pi)_("tr",Q_"tr") (xi_(bold(sigma),j) bold(T)_alpha))_(beta gamma):
                        0 <= j <= N, 1 <= alpha <= n_s, 1 <= beta, gamma <= d} \
                    & quad union {tilde(Pi)_(D,K,Q_"R")^(1) xi_(bold(u),j): 0 <= j <= N},
$
板弯曲相应为
$
  cal(B)_("KL",N,K) & := {(xi_(bold(M),j) bold(T)_alpha)_(beta gamma):
                        0 <= j <= N, 1 <= alpha <= 3, 1 <= beta, gamma <= 2} \
                    & quad union {tilde(Pi)_(D,K,Q_"R")^(2) xi_(w,j): 0 <= j <= N}.
$
...
#proof[
  第一步：分块。条件于两组投影规则后，$cal(D)_(N,K)$ 按 $cal(B)_(N,K)$ 的两个并项分为两族。未投影块由应力（弯矩）分量的至多 $m$ 阶导数组成：$tilde(Pi)_("tr",Q_"tr")$ 只减去一个由独立样本定出的常数球张量，条件化后是固定平移，其散度为零且不改变任何导数，故该族仍是脊函数族 $bb(D) = {rho_k (bold(omega) dot bold(x) + b) : (bold(omega), b) in bb(S)^(d-1) times [-c_b, c_b]}$ 的像。已投影块是位移（$m=1$）与挠度（$m=2$）特征，经 $tilde(Pi)_(D,K,Q_"R")^(m)$ 后落在 $V_K^(m)$ 中，是 B 样条而非脊函数。

  第二步：未投影块。参数域紧，$rho_k in W^(k,oo)$（其 $k-1$ 阶导数 Lipschitz），故由 $k >= m+1$ 有 $rho_k in W^(m+1,oo)$，@SiegelHongJinHaoXu2023 定理 6 适用：
  $
    frak(R)_Q (partial^bold(alpha) bb(D)) lt.tilde Q^(-1\/2),
    quad abs(bold(alpha)) <= m,
  $
  隐含常数与 $Q$、$N$ 无关（但依赖于 $k$，因 $rho_k$ 的 Lipschitz 常数随 $k$ 增大）。该定理的证明只用到 $partial^bold(alpha) bb(D) = {bold(omega)^bold(alpha) rho_k^((bold(alpha)))(bold(omega) dot bold(x) + b)}$ 与 Lipschitz 复合规则，正是脊结构所在。该块在两个残差中以柔度算子的分量与 ${bold(T)_alpha}$ 的分量为系数出现，前者以 $1\/mu$（板弯曲为 $6\/(mu h^3)$）为界且关于 $lambda$ 一致，故取有限个这样的线性组合不改变上述阶。
  ...
  $
  最后由对称化（@SiegelHongJinHaoXu2023 定理 4）将 $frak(R)_Q$ 转为一致偏差的期望，并代入 @eq:envelope-rate，即得结论。逆估计常数 $C_("inv",m)$ 与 B 样条次数、网格准均匀性有关，与 Lamé 常数无关，故隐含常数关于 $lambda$ 一致。证毕。
]
...
#theorem(title: [线弹性线性化网络经验最小二乘误差])[
  假设 @thm:elasticity-stability 成立，单隐层参数取上述准均匀字典。设 $bold(sigma)_star$ 与 $bold(u)_star$ 的各分量分别属于 $H^(s_(bold(sigma)))(Omega)$ 与 $H^(s_(bold(u)))(Omega)$，$s_chi in [1, s_"cap" (d)]$，且预算 $B$ 足以容纳一个达到所述逼近阶的比较函数。固定 $0<delta,eta<1$，并设 $Q_"R"$ 满足 @thm:rquad-rate 的谱稳定样本条件。期望对训练样本、平均迹规则与 Ritz 规则取，其中 Ritz 规则条件于 $cal(E)_("R",delta)$，则
  $
...
#proof[
  ...
  $
  记 $bold(c)^"out" in cal(C)_(N,B)$ 为求解器输出的系数，则计算所得物理解为 $bold(z)_(N,Q,K) = tilde(bold(z))(bold(c)^"out")$。由 $k >= 3$ 有 $rho_k in C^2$ 且其二阶导数 Lipschitz，原始特征逐分量属于 $W^(k,oo)(Omega) subset H^3(Omega)$，故上述诸场均属于 $bold(H)(div) times H^1(Omega; RR^d)$。下文将乘积图范数
  $
  ...
  第四步：比较函数。由正则性假设，$bold(sigma)_star$ 的各分量属于 $H^(s_(bold(sigma)))(Omega)$，$bold(u)_star$ 的各分量属于 $H^(s_(bold(u)))(Omega)$，$s_chi in [1, s_"cap" (d)]$。在固定基 ${bold(T)_alpha}$ 与 ${bold(e)_i}$ 下将两个场逐分量分解，对每个标量分量在相应变量的字典（含常数特征与多项式方向补足）上应用 @thm:dictionary-rate，得到原始字典场 $hat(bold(tau))^"cmp" in hat(bold(Sigma))_("LE",N)$ 与 $hat(bold(v))^"cmp" in hat(bold(U))_("LE",N)$，满足
  $
  ...
  $
  其中用了 $m_N lt.tilde N$。预算假设正是保证 $B$ 不小于该量级，故 $bold(c)^"cmp" in cal(C)_(N,B)$。$bold(c)^"cmp"$ 由确定性字典与精确解决定，与训练样本无关。
  ...
  $
  <eq:comparison-bound>
  其中 $C_2$ 依赖于 $Omega$、$c_b$、$d$ 与精确解分量的 Sobolev 范数，不依赖于 $N$、$Q$、$K$。
  ...
  $
    ...
    + epsilon_"opt",
  $
  隐含常数只依赖于 $c_1$、$C_1$ 与 $C_2$，不依赖于 $N$、$Q$、$K$ 与 $B$。由 $bold(u)_star$ 各分量属于 $H^(s_(bold(u)))(Omega)$ 与 @eq:ritz-rate，
  $
    epsilon_"Ritz" (K)^2
...
#theorem(title: [板弯曲线性化网络经验最小二乘误差])[
  假设 @thm:plate-stability 成立，单隐层参数取上述准均匀字典。设 $bold(M)_star$ 与 $w_star$ 的各分量分别属于 $H^(s_(bold(M)))(Omega)$ 与 $H^(s_w)(Omega)$，$s_chi in [2, s_"cap" (2)]$，且预算 $B$ 足以容纳一个达到所述逼近阶的比较函数。固定 $0<delta,eta<1$，并设 $Q_"R"$ 满足 @thm:rquad-rate 的谱稳定样本条件。期望对训练样本与条件于 $cal(E)_("R",delta)$ 的 Ritz 规则取，则
  $
...
#proof[
  ...
  $
  由 $k >= 3$，原始特征逐分量属于 $H^3(Omega)$，故诸场属于 $bold(H)(div div) times H^2(Omega)$。即 @fig:least-squares-projection-paths 中两条路径只作用于挠度分量，弯矩块沿两路都保持为 $hat(bold(tau))_(bold(c))$。$bold(Sigma)_"KL"$ 不带规范约束且 $V_K^(2) subset H_0^2(Omega)$，故精确投影场自动属于 $bold(X)_"KL"$。第二步以 @thm:plate-stability 两侧的隐含常数替代 $c_1$、$C_1$ 后逐字重复，其上界证明同样未用边界条件，连续性在全空间 $bold(H)(div div) times H^2$ 成立。第三步只出现 $epsilon_"Rquad" <= epsilon_"projquad"$。第四步删去 $Pi_"tr"$ 一段，弯矩图范数由逐分量 $H^2$ 范数经 $norm(div div bold(w))_(L^2(Omega)) lt.tilde norm(bold(w))_(H^2(Omega))$ 控制，Ritz 误差取 $epsilon_"Ritz" (K) := norm((I - Pi_(D,K)^(2)) w_star)_(H^2(Omega))$，由 $w_star in H^(s_w)(Omega)$ 与 @eq:ritz-rate 得 $epsilon_"Ritz" (K) lt.tilde K^(-(min(s_w, p+1)-2)\/2)$。字典逼近率给出 $N^(-2 beta_"KL")$，其中 $m = 2 <= k$ 满足 @thm:dictionary-rate 的导数阶限制。第五、六步不变。最后对条件于 $cal(E)_("R",delta)$ 的 Ritz 规则取期望并应用 @thm:rquad-rate，以 $B^2 K Q_"R"^(-1)$ 替换条件估计中的 $epsilon_"Rquad"^2$，即得结论。证毕。
]
...
#figure(
  three-line-table(
    ...
    |---|---|
    | 计算区域 | $Omega=(0,1)^d$ |
    | 参数点集 | 满足 @def:quasi-uniform 的准均匀点集，偏置半径 $c_b=2$ |
...
#figure(
  three-line-table(
    ...
    | Lamé 常数 | $lambda=1$ |
    | 激活幂次 | $k=7$ |
    | 字典规模 | $N in {200,400,600,800,1000}$ |
    | 训练求积 | $Q=4(N+1)$ |
...
#figure(
  ...
  caption: [固定 $k=7$ 时二维线弹性的应力与位移收敛曲线。虚线以 @eq:beta-rate 的理论指数为参考，误差棒为十次独立重复的样本标准差],
...
#figure(
  three-line-table(
    ...
    |---|---|---|
    | $200$ | $3.56 times 10^(-6)$ | $6.41 times 10^(-7)$ |
    | $400$ | $1.76 times 10^(-7)$ | $3.76 times 10^(-8)$ |
    | $600$ | $5.22 times 10^(-8)$ | $1.13 times 10^(-8)$ |
    | $800$ | $1.01 times 10^(-8)$ | $4.26 times 10^(-9)$ |
    | $1000$ | $2.79 times 10^(-9)$ | $5.79 times 10^(-10)$ |
  ],
  caption: [固定 $k=7$ 时二维线弹性在各 $N$ 处的误差均值],
)<tbl:ls-order-elasticity-2d>

$k=7$ 时应力与位移实际误差的观测阶分别为 $4.32$ 与 $4.05$，均达到并超过字典逼近项的理论保证 $beta(7, 2, 1)=3.75$。

=== 关于幂次 $k$ 的收敛实验

本实验对 $k in {3,5,7,9}$ 分别运行完整经验最小二乘算法，字典规模固定为 $N=500$。训练求积与辅助空间次数按 @eq:degree-match 与最高幂次 $k=9$ 相匹配，材料参数与关于 $N$ 的收敛实验相同，全部离散配置见 @tbl:ls-config-power-elasticity-2d。
...
#figure(
  ...
  caption: [固定 $N=500$ 时二维线弹性完整经验最小二乘解的图范数误差随激活幂次的变化。误差棒表示十次独立重复的样本标准差，虚线是忽略依赖于 $k$ 的常数后的 $N^(-(beta(k)-beta(3)))$ 等常数启发线，不是固定 $N$ 下的定量预测],
...
#figure(
  ...
  caption: [固定 $N=500$ 时二维线弹性在各 $k$ 处的图范数误差均值],
)<tbl:ls-power-elasticity-2d>

@fig:ls-convergence-power-elasticity-2d 给出固定 $N=500$ 时应力与位移图范数误差随幂次 $k$ 的变化，@tbl:ls-power-elasticity-2d 列出各幂次处的误差均值。相邻幂次的位移均值分别下降约 $182$、$142$ 与 $47$ 倍，应力均值分别下降约 $242$、$218$ 与 $30$ 倍。
...
#figure(
  ...
  caption: [固定 $k=7$ 时平面应力的应力与位移收敛曲线。虚线以 @eq:beta-rate 的理论指数为参考，误差棒为十次独立重复的样本标准差],
...
#figure(
  three-line-table(
    ...
    | $N$ | 应力 $bold(H)(div)$ 误差 | 位移 $H^1$ 误差 |
    |---|---|---|
    | $200$ | $2.82 times 10^(-6)$ | $6.42 times 10^(-7)$ |
    | $400$ | $1.41 times 10^(-7)$ | $3.79 times 10^(-8)$ |
    | $600$ | $4.26 times 10^(-8)$ | $1.15 times 10^(-8)$ |
    | $800$ | $7.82 times 10^(-9)$ | $4.26 times 10^(-9)$ |
    | $1000$ | $2.15 times 10^(-9)$ | $5.78 times 10^(-10)$ |
  ],
  caption: [固定 $k=7$ 时平面应力在各 $N$ 处的误差均值],
)<tbl:ls-order-plane-stress>

应力与位移误差的观测阶分别为 $4.34$ 与 $4.05$，各 $N$ 处的误差也与二维线弹性接近，因而在所考察的参数范围内，没有观察到本构张量改变引起的附加退化。
...
#figure(
  three-line-table(
    ...
    | 激活幂次 | $k=7$ |
    | 字典规模 | $N in {200,400,600,800,1000}$ |
    | 训练求积 | $Q=16(N+1)$ 个均匀 Monte Carlo 点 |
    | 辅助空间 | $p=7$，目标维数 $2(N+1)$，实际 $K in {512,1000,1331,1728,2197}$ |
...
#figure(
  ...
  caption: [固定 $k=7$ 时三维线弹性的应力与位移收敛曲线。虚线以 @eq:beta-rate 的理论指数为参考，误差棒为十次独立重复的样本标准差],
...
#figure(
  three-line-table(
    ...
    | $N$ | 应力 $bold(H)(div)$ 误差 | 位移 $H^1$ 误差 |
    |---|---|---|
    | $200$ | $3.49 times 10^(-7)$ | $1.73 times 10^(-8)$ |
    | $400$ | $2.63 times 10^(-7)$ | $2.98 times 10^(-9)$ |
    | $600$ | $9.07 times 10^(-8)$ | $1.06 times 10^(-9)$ |
    | $800$ | $4.03 times 10^(-8)$ | $2.63 times 10^(-9)$ |
    | $1000$ | $3.40 times 10^(-8)$ | $1.87 times 10^(-9)$ |
  ],
  caption: [固定 $k=7$ 时三维线弹性在各 $N$ 处的误差均值],
)<tbl:ls-order-elasticity-3d>

五点对数最小二乘拟合给出的应力与位移观测阶分别为 $1.58$ 与 $1.35$。两条曲线在大 $N$ 端均不再保持理论参考线的斜率，其中位移误差从 $N=600$ 起处于 $10^(-9)$ 量级的平台。因此这组完整算法结果验证了三维离散流程随字典加密总体收敛，但不能据此声称达到了字典逼近项的理论指数 $beta(7,3,1)=8/3$。
...
#figure(
  three-line-table(
    ...
    | 字典规模 | $N_sigma=N_u=N in {200,400,600,800,1000}$ |
    | 激活幂次 | $k=7$ |
    | 辅助空间 | 次数 $p=9$，目标维数 $2(N+1)$，实际 $K in {441,841,1225,1681,2025}$ |
    | 训练求积 | $Q=8(N+1)$ 个均匀 Monte Carlo 点 |
...
#figure(
  three-line-table(
    ...
    |---|---|---|---|
    | $200$ | $2.591 times 10^(-3)$ | $2.600 times 10^(-3)$ | $2.600 times 10^(-3)$ |
    | $400$ | $8.940 times 10^(-5)$ | $8.960 times 10^(-5)$ | $8.960 times 10^(-5)$ |
    | $600$ | $2.223 times 10^(-5)$ | $2.232 times 10^(-5)$ | $2.232 times 10^(-5)$ |
    | $800$ | $6.648 times 10^(-6)$ | $6.659 times 10^(-6)$ | $6.658 times 10^(-6)$ |
    | $1000$ | $1.508 times 10^(-6)$ | $1.514 times 10^(-6)$ | $1.514 times 10^(-6)$ |
  ],
...
#figure(
  three-line-table(
    ...
    | $N$ | $lambda=10$ | $lambda=10^3$ | $lambda=10^5$ |
    |---|---|---|---|
    | $200$ | $3.394 times 10^(-4)$ | $3.432 times 10^(-4)$ | $3.433 times 10^(-4)$ |
    | $400$ | $1.221 times 10^(-5)$ | $1.232 times 10^(-5)$ | $1.232 times 10^(-5)$ |
    | $600$ | $4.098 times 10^(-6)$ | $4.123 times 10^(-6)$ | $4.123 times 10^(-6)$ |
    | $800$ | $7.103 times 10^(-7)$ | $7.152 times 10^(-7)$ | $7.161 times 10^(-7)$ |
    | $1000$ | $2.330 times 10^(-7)$ | $2.396 times 10^(-7)$ | $2.404 times 10^(-7)$ |
  ],
...
#figure(
  ...
  caption: [固定 $k=7$ 时 Kirchhoff--Love 板弯曲的弯矩与挠度收敛曲线。虚线以 @eq:beta-rate 的理论指数为参考，误差棒为十次独立重复的样本标准差],
...
#figure(
  three-line-table(
    ...
    |---|---|---|
    | $200$ | $1.44 times 10^(-2)$ | $2.18 times 10^(-3)$ |
    | $400$ | $7.89 times 10^(-4)$ | $1.13 times 10^(-4)$ |
    | $600$ | $2.60 times 10^(-4)$ | $4.23 times 10^(-5)$ |
    | $800$ | $7.92 times 10^(-5)$ | $1.25 times 10^(-5)$ |
    | $1000$ | $2.22 times 10^(-5)$ | $3.21 times 10^(-6)$ |
  ],
  caption: [固定 $k=7$ 时板弯曲在各 $N$ 处的误差均值],
)<tbl:ls-order-plate>

$k=7$ 时弯矩与挠度实际误差的观测阶分别为 $3.87$ 与 $3.86$，均高于板图范数的理论基准 $beta(7, 2, 2)=3.25$。

=== 关于幂次 $k$ 的收敛实验

本实验对 $k in {3,5,7,9}$ 分别运行完整经验最小二乘算法，字典规模固定为 $N=1000$。训练求积与辅助空间次数按 @eq:degree-match 与最高幂次 $k=9$ 相匹配，材料参数与关于 $N$ 的收敛实验相同，全部离散配置见 @tbl:ls-config-power-plate。
...
#figure(
  ...
  caption: [固定 $N=1000$ 时板弯曲完整经验最小二乘解的图范数误差随激活幂次的变化。误差棒表示十次独立重复的样本标准差，虚线是忽略依赖于 $k$ 的常数后的 $N^(-(beta(k)-beta(3)))$ 等常数启发线，不是固定 $N$ 下的定量预测],
...
#figure(
  ...
  caption: [固定 $N=1000$ 时板弯曲在各 $k$ 处的图范数误差均值],
)<tbl:ls-power-plate>

@fig:ls-convergence-power-plate 给出固定 $N=1000$ 时弯矩与挠度图范数误差随幂次 $k$ 的变化，@tbl:ls-power-plate 列出各幂次处的误差均值。相邻幂次的弯矩均值分别下降约 $119$、$135$ 与 $6.3$ 倍，挠度均值分别下降约 $145$、$116$ 与 $3.5$ 倍。四个幂次下实际误差持续下降。此外，$k=3$ 与 $k=7$ 下 $N$ 的完整取值序列分别给出 $1.22$ 与 $3.86$ 的挠度实际观测阶。

= 结语

本文的主要结果是线弹性与板弯曲的两条残差稳定性定理：混合最小二乘泛函与相应图范数平方双边等价，且等价常数关于 Lamé 常数 $lambda$ 一致，在近不可压缩极限下不退化。在此基础上，本文将参数准均匀的单隐层线性化 $rho_k$ 网络用于两类方程的数值求解，并给出误差分析。字典逼近项是确定性的 $N^(-beta)$，没有独立抽样带来的对数修正，而有限维计算必须显式计入 Ritz 截断与投影积分误差。
