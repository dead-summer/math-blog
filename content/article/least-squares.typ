...
在具体理论上，本文分别建立了该方法在线弹性应力-位移混合形式与 Kirchhoff-Love 板弯曲弯矩-挠度混合形式下的连续与离散分析。理论结果表明，只要离散空间与相应连续问题的函数空间保持一致，连续稳定性、离散稳定性以及准最优误差估计便可为该方法提供一致的分析基础。数值实验进一步在三维线弹性、二维线弹性、近不可压缩三维线弹性、平面应力与 Kirchhoff-Love 板弯曲算例上验证了该方法的可行性；同时，利用对称结构并进行谱截断的 TSVD 求解版本整体更稳健，而通用 Lstsq 对特征数量增加导致的代数病态更敏感。
