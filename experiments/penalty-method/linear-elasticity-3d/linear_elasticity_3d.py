...
sys.modules.setdefault("linear_elasticity_3d", sys.modules[__name__])
...
ALGO_STYLE = {
    ...
    "PINN": {"color": "#E76F51", "marker": "X", "linestyle": "-"},
}
...
class SharedComparisonConfig:
    ...
    pinn_init_seed: int = BASE_SEED + 4
...
def validate_shared_comparison_config(cfg: SharedComparisonConfig) -> None:
    ...
    if cfg.Q_bc < 6:
        raise ValueError("SharedConfig.Q_bc must be at least 6.")
    if cfg.Q_test <= 0:
...
def build_compliance_matrix(E: float, nu: float) -> torch.Tensor:
    """Build the 6x6 isotropic compliance matrix in Voigt form."""
...
def sample_boundary_points(
    ...
    """Sample points on the six faces of the unit cube with quadrature weights."""
...
def compute_feature_capacity(M_s: int, M_u: int) -> int:
    """Return the total trainable coefficients of the split feature model."""
...
def compute_coefficient_residual_norms(
    ...
    """Evaluate strong-form residual norms on sampled interior/boundary points."""
...
def apply_shared_to_pinn_config(
    cfg: "PinnConfig",
    shared_cfg: SharedComparisonConfig,
...
def main(cfg: MainConfig | None = None) -> None:
    """Run the selected algorithms across the three experiment families."""
    ...
    if any(algorithm_id in feature_algorithm_ids for algorithm_id in selected_algorithm_ids):
    ...
    if "pinn" in selected_algorithm_ids:
        ...
        from pinn import run_experiment as run_pinn_experiment

        pinn_cfg, target_capacity, param_count = apply_shared_to_pinn_config(
            cfg.pinn,
            shared_cfg,
        )
        print(
