...
def accumulate_weak_form_moments(
    ...
    cross_u_grad_s = [
        torch.zeros(mp1_u, mp1_s, dtype=DTYPE, device=DEVICE) for _ in range(2)
    ]
    force_moment = torch.zeros(mp1_u, 2, dtype=DTYPE, device=DEVICE)
...
def assemble_stress_matrix(
    ...
    dim_s = 3 * gram_s.shape[0]
    A = torch.zeros(dim_s, dim_s, dtype=DTYPE, device=DEVICE)
    for row in range(3):
        for col in range(3):
            A[row::3, col::3] = compliance_voigt[row, col] * gram_s
    return A
...
def assemble_coupling_matrix(cross_u_grad_s: list[torch.Tensor]) -> torch.Tensor:
    ...
    D = [cross_u_grad_s[dim].T for dim in range(2)]
...
def assemble_rhs_vector(force_moment: torch.Tensor) -> torch.Tensor:
    ...
    F = torch.zeros(2 * force_moment.shape[0], dtype=DTYPE, device=DEVICE)
    for comp in range(2):
        F[comp::2] = force_moment[:, comp]
    return F
